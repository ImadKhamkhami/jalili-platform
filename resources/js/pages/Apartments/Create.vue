...
const terraceTotal = computed(() => {
  if (!form.terrace_area) return 0
  return Number(form.terrace_area) * (Number(form.price_per_m2 || 0) / 2)
})
...
function submit() {
  form.post('/apartments', { forceFormData: true })
}
</script>
...
<div class="mt-6 bg-white p-6 rounded-2xl shadow max-w-7xl mx-auto">
  <button @click="submit" class="bg-green-700 text-white px-10 py-3 rounded-xl">
    حفظ الشقة
  </button>
</div>
